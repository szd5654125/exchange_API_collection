...
from apis.hyperliquid.exchange import Exchange
from apis.hyperliquid.info import Info


def setup():
    account: LocalAccount = eth_account.Account.from_key(config['exchanges']['hyperliquid']["secret_key"])
    address = config['exchanges']['hyperliquid']["account_address"]
    if address == "":
    ...
    if float(margin_summary["accountValue"]) == 0 and len(spot_user_state["balances"]) == 0:
        ...
        error_string = f"No accountValue:\nIf you think this is a mistake, make sure that {address} has a balance.\nIf address shown is your API wallet address, update the config to specify the address of your account, not the address of the API wallet."
        raise Exception(error_string)
    exchange = Exchange(account, account_address=address)
